...
class TestManager:
    ...
    def one_epoch(self, split, basename_results, nb_augmentations):
        ...
        for it, batch in enumerate(tqdm(loader.dataset, bar_format=bar_format)):
            ...
            if split == "val":
                y_gt.append(batch[1])
                # devices.append(batch[3]) # We don't include the label in our studies, we only want to check the training time
    ...
    def load_state(self, merge = True):
        """
    ...
    def test(self, basename_results=None, nb_augmentations=0, merge_bn = True):
        """
        ...
        :return:
        """
        self.load_state(merge=merge_bn)
